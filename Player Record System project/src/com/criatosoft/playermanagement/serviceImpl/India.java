package com.criatosoft.playermanagement.serviceImpl;
...
import com.criatosoft.playermanagement.model.Player;
import com.criatosoft.playermanagement.service.World;
		...
		if (count>0)
		{
		    ...
		    for(int i=0;i<count;i++)
			  ...
			  if(num==plrno)
			 {
			  ...
			  while(score<1000)
			  {
				  System.out.println("please enter minimum 1000score");
		...
		if (count>0)
		{
		    ...
		    for(int i=0;i<count;i++)
			 ...
			 int plrno=p[i].getPrank();
			  if(num==plrno)
			  {
		       System.out.println("player rank is "+p[i].getPrank());
		       br.readLine();
		      }
